...
import static sun.font.CharToGlyphMapper.isVariationSelectorSupported;
import static sun.font.CharToGlyphMapper.isVariationSelector;
import static sun.font.CharToGlyphMapper.isVariationSelectorInBMP;
import static sun.font.CharToGlyphMapper.isVariationSelectorInSupplementary;
import static sun.font.CharToGlyphMapper.isBaseChar;
                ...
                try {
                    ...
                    if (dot != mark) {
                        ...
                        if (dot > 1) {
                            ...
                            // both characters of base and variation selector should be deleted.
                            if (isVariationSelectorSupported()) {
                                int vsCount = 0;
                                int vsSize = 0;
                                if (dot > 2 &&
                                    isVariationSelectorInSupplementary(c0, c1)) {
                                    vsCount = 1;
                                    vsSize = 2;
                                }else if (isVariationSelectorInBMP(c1)) {
                                    vsCount = 1;
                                    vsSize = 1;
                                }
                                if (vsCount == 1){
                                    String targetChars = doc.getText(0,dot);
                                    int pre = targetChars.codePointBefore(dot-vsSize);
                                    int preBase = pre;
                                    while(isVariationSelector(preBase)) {
                                        vsCount++;
                                        vsSize++;
                                        if (preBase >= 0x10000){
                                            vsSize++;
                                        ...
                                        preBase = targetChars.codePointBefore(dot - vsSize);
                                    }                                           //IBM-T6_input_method_framework
                                    if (preBase == 0 ||                         //IBM-T6_input_method_framework
                                        isBaseChar(preBase) == false){
                ...
                try {
                    ...
                    if (dot != mark) {
                        ...
                        if (dot < doc.getLength() - 1) {
                            ...
                            if (isVariationSelectorSupported() &&
                                isBaseChar(baseChar) &&
                                dot < doc.getLength() -delChars) {
                                String nextChar = doc.getText(dot+delChars, 1);
                                char c2 = nextChar.charAt(0);
                                if (isVariationSelectorInBMP(c2)) {
                                    delChars += 1;
                                }else if (dot < doc.getLength() -delChars -1) {
                                    nextChar = doc.getText(dot+delChars+1, 1);
                                    char c3 = nextChar.charAt(0);
                                    if (isVariationSelectorInSupplementary(c2,c3)) {
                                        delChars += 2;
                                    }
                                }
                            }
