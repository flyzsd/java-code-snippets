    ...
    implements Comparable<Charset>
{
    ...
    private static volatile Charset bootstrapCharset;
    ...
    public static Charset defaultCharset() {
        if (defaultCharset == null) {
            ...
            if (bootstrapCharset != null && !sun.misc.VM.isBooted()) return bootstrapCharset;
            ...
            synchronized (Charset.class) {
                ...
                else
                {
                    if ("z/OS".equals((String)AccessController.doPrivileged(new GetPropertyAction("os.name")))) {
                        sun.misc.MessageUtils.toStderr(
                            "WARNING: Specified charset is not supported, IBM1047/Cp1047 is used for default charset\n");
                        if (sun.misc.VM.isBooted()) {
                            defaultCharset = forName("IBM1047");
                        }
                        else {
                            bootstrapCharset = forName("IBM1047");
                        }
                    } else {
                        sun.misc.MessageUtils.err(
                            "WARNING: Specified charset is not supported, UTF-8 is used for default charset");
                        defaultCharset = forName("UTF-8");
                    }
                }
            }
            if (defaultCharset == null) return bootstrapCharset;
        }
