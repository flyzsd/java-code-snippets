 ...
 */
private static boolean checkPermissionHelper(Permission perm, AccessControlContext acc, Object[] objects, int frame, Object[] checked, Object[] objPDomains, int debug, int startPos) {
	...
	if (2 == startPos) {	// Pre-JEP140 format
		if (null != acc && (null != acc.doPrivilegedAcc || null != acc.nextStackAcc || acc.isLimitedContext)) {
			checked = new Object[OBJS_ARRAY_SIZE];	/* checked was null initially when Pre-JEP140 format */
			return AccessControlContext.checkPermissionWithCache(perm, (Object[])pDomains, debug, acc, false, null, null, checked);
		} else {
		...
		if (AccessControlContext.checkPermissionWithCache(perm,
				(Object[])pDomains, debug, (AccessControlContext)objects[frame*OBJS_ARRAY_SIZE],
...
private static boolean debugHelperPreJEP140(Object[] objects, Permission perm) {
	...
	if (AccessControlContext.debugCodeBaseArray != null) {
		...
		for (int i = 2; i < objects.length; i++) {
			ProtectionDomain pd = (ProtectionDomain)objects[i];
	...
	if (AccessControlContext.debugCodeBaseArray != null) {
		...
		for (int j = 0; j < objects.length/OBJS_ARRAY_SIZE; j++) {
			...
			for (int i = 1; i < objPDomains.length; i++) {
				ProtectionDomain pd = (ProtectionDomain)objPDomains[i];
				CodeSource cs = null == pd ? null : pd.getCodeSource();
				if ((cs != null) && AccessControlContext.debugCodeBase(cs.getLocation()))  {
					debug = true;
					break done;
...
public static void checkPermission(Permission perm) throws AccessControlException {
	...
	if (AccessControlContext.debugSetting() != 0) {
		if (null == (String)DebugRecursionDetection.getTlDebug().get()) {
	...
	if (isPreJEP140Format) {
		...
		Object[]	checked = new Object[OBJS_ARRAY_SIZE];
...
/**
 * This is a helper method for getContext() & doPrivilegedWithCombiner methods.
...
/**
 * Helper method to determine the newAuthorizedState according to incoming acc & callerPD
 *
 * @param	acc the AccessControlContext object to be checked
...
private static ProtectionDomain[] toArrayOfProtectionDomains(Object[] domains, AccessControlContext acc, int startPos) {
	int len = 0;
	int extra = 0;
	int size = (null == domains) ? 0 : domains.length;
