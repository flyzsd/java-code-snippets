...
public final class NetworkInterface {
	...
	private static boolean zOS = false;

    static {
        AccessController.doPrivileged(
            new java.security.PrivilegedAction<Void>() {
				GetPropertyAction propa = new GetPropertyAction("os.name");
				String osName = (String)AccessController.doPrivileged(propa);
                public Void run() {
    ...
    public boolean equals(Object obj) {
		...
		/* Interfaces generated by dynamic XCF use a single IP address on z/OS
		 * So, we can skip equality test on automatically generated interface names
		 */ 
        if (this.name != null ) {
            if (!this.name.equals(that.name) &&
					!(zOS && (this.name.startsWith("EZAXCF") || // IPv4 XCF interface
						that.name.startsWith("EZAXCF") ||
						this.name.startsWith("EZ6XCF") ||       // IPv6 XCF interface
						that.name.startsWith("EZ6XCF") ||
						this.name.equals("EZASAMEMVS") ||       // IPv4 same host interface 
						that.name.equals("EZASAMEMVS") ||
						this.name.equals("EZ6SAMEMVS") ||       // IPv6 same host interface
						that.name.equals("EZ6SAMEMVS") ||
						this.name.startsWith("IQDIOLNK") ||     // IPv4 Hipersockets interface
						that.name.startsWith("IQDIOLNK") ||
						this.name.equals("IQDIOINTF6") ||       // IPv6 Hipersockets interface
						that.name.equals("IQDIOINTF6")))) {
                return false;
            }
        } else {
            if (that.name != null) {
