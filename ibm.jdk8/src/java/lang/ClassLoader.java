...
public abstract class ClassLoader {
	...
	private final Object assertionLock = new AssertionLock();
  	...
  	private volatile Hashtable<String, Package> packages;
	...
	private static Map<Object, Object> parallelCapableCollection;
	...
	static final void initializeClassLoaders() {
		...
		String allowValue = System.getProperty("sun.lang.ClassLoader.allowArraySyntax"); //$NON-NLS-1$
		if (allowValue != null) allowValue = allowValue.toLowerCase();
		if ("true".equals(allowValue)) { //$NON-NLS-1$
			allowArraySyntax = true;
		}

		String propValue = System.getProperty("reflect.cache"); //$NON-NLS-1$
		...
		String systemLoaderString = System.getProperty("systemClassLoader"); //$NON-NLS-1$
		...
		String lazyValue = System.getProperty("java.lang.ClassLoader.lazyInitialization"); //$NON-NLS-1$
		if (lazyValue != null) {
...
public Enumeration<URL> getResources(String resName) throws IOException {
	...
	if (parent != null) {
		...
		resources = bootstrapClassLoader.getResources(resName);
	}
		...
		synchronized(classLoaderClass) {
			if (initSystemClassLoader) {
				initSystemClassLoader = false;
					...
					String userLoader = System.getProperty("java.system.class.loader"); //$NON-NLS-1$
					if (userLoader != null) {
						try {
...
protected Package getPackage(String name) {
	...
	if (this != bootstrapClassLoader) {
		...
		return parentLoader.getPackage(name);
	}
	return null;
}
...
protected Package[] getPackages() {
	...
	Hashtable<?, Package> localPackages;
	synchronized(lazyInitLock) {
	...
	do {
		try {
        	...
        	cloned = true;
            localPackages = (Hashtable<?, Package>)localPackages.clone();
		}
	} while (true);
}
	...
	throws IllegalArgumentException
{
	...
	synchronized(packages) {
		...
		} else throw new IllegalArgumentException(com.ibm.oti.util.Msg.getString("K0053", name)); //$NON-NLS-1$
	}
}
 ...
 * @return	true when all the certs in one array are present in the other arrary
 * 			false otherwise
 */
