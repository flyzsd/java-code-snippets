...
public final class System {
	...
	static void afterClinitInitialization() {
		...
		if (com.ibm.oti.vm.VM.PACKED_SUPPORT_ENABLED) {
			sun.reflect.Reflection.registerMethodsToFilter(com.ibm.jvm.packed.reflect.PackedUnsafe.class, "getUnsafe"); //$NON-NLS-1$
		}
		...
		try {
			java.lang.reflect.Field f = J9VMInternals.class.getDeclaredField("jitHelpers"); //$NON-NLS-1$
			sun.misc.Unsafe unsafe = sun.misc.Unsafe.getUnsafe();
			unsafe.putObject(unsafe.staticFieldBase(f), unsafe.staticFieldOffset(f), com.ibm.jit.JITHelpers.getHelpers());
		 ...
		 */
		String enableCopyProperty = getProperty("java.lang.string.constructor.copy.substrings"); //$NON-NLS-1$
		String.enableCopy = enableCopyProperty == null || !enableCopyProperty.equalsIgnoreCase("false"); //$NON-NLS-1$
       ...
       */
		...
		String.disableCopyInSubstring = disableCopyInSubstringProperty != null && disableCopyInSubstringProperty.equalsIgnoreCase("true"); //$NON-NLS-1$
...
private static void ensureProperties() {
	...
	systemProperties.put("java.version", "1.8.0"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	systemProperties.put("com.ibm.oti.jcl.build", "20160915_318796"); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 */
	String thisVersion = "\nJ9CL - " + systemProperties.get("com.ibm.oti.jcl.build"); //$NON-NLS-1$ //$NON-NLS-2$
	String fullVersion = systemProperties.getProperty("java.fullversion"); //$NON-NLS-2$
	if(fullVersion != null) {
		systemProperties.put("java.fullversion", fullVersion + thisVersion); //$NON-NLS-1$
	}
	String info = systemProperties.getProperty("java.vm.info"); //$NON-NLS-1$
	if(info != null) {
		systemProperties.put("java.vm.info", info + thisVersion); //$NON-NLS-1$
	}

	lineSeparator = getProperty("line.separator", null); //$NON-NLS-1$
}
/**
