...
public class ThreadInfo {
    ...
    public static ThreadInfo from(CompositeData cd) {
        ...
        if (cd != null) {
            ...
            try {
                ...
                lockNameVal = cd.get("lockName") != null ? (String) cd //$NON-NLS-1$
                        .get("lockName") : null; //$NON-NLS-1$
                lockOwnerIdVal = ((Long) cd.get("lockOwnerId")).longValue(); //$NON-NLS-1$
                lockOwnerNameVal = cd.get("lockOwnerName") != null ? (String) cd //$NON-NLS-1$
                        .get("lockOwnerName") //$NON-NLS-1$
                        : null;
                CompositeData[] stackTraceDataVal = (CompositeData[]) cd
                        .get("stackTrace"); //$NON-NLS-1$
    ...
    private static LockInfo[] recoverLockedSynchronizers(CompositeData cd) {
        ...
        try {
            CompositeData[] lockedSynchronizersDataVal = (CompositeData[]) cd
                    .get("lockedSynchronizers"); //$NON-NLS-1$
    ...
    private static LockInfo recoverLockInfoAttribute(CompositeData cd,
        ...
        try {
            CompositeData lockInfoCDVal = cd.get("lockInfo") != null ? (CompositeData) cd //$NON-NLS-1$
                    .get("lockInfo") //$NON-NLS-1$
                    : null;
            if (lockInfoCDVal != null) {
                ...
                result = new LockInfo(
                        ...
                        ((Integer) lockInfoCDVal.get("identityHashCode")) //$NON-NLS-1$
                                .intValue());
            }// end if
        } catch (InvalidKeyException e) {
