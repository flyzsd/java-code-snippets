...
public abstract class MethodHandle {
	...
	public MethodHandle asCollector(Class<?> arrayClass, int collectCount) throws IllegalArgumentException, WrongMethodTypeException, NullPointerException {
		...
		Class<?> lastClass = type.lastParameterType();
		...
		return new CollectHandle(asType(type.changeParameterType(parameterCount - 1, arrayClass)), collectCount);
	}
	...
	public Object invokeWithArguments(Object... args) throws Throwable, WrongMethodTypeException, ClassCastException {
		 ...
		 */
		if (args.length != type.arguments.length) {
			throw new WrongMethodTypeException();
		}
		if (args.length < 253) {
			return IWAContainer.getMH(type.arguments.length).invokeExact(this, args);
		}
		return this.asSpreader(Object[].class, args.length).invoke(args);
	}
		...
		if ((value != null) && this instanceof IndirectHandle) {
			try {
				...
				if (this instanceof InterfaceHandle) {
					if ((result.rawModifiers & Modifier.PUBLIC) == 0) {
	 ...
	 */
	abstract void compareWith(MethodHandle other, Comparator c);
