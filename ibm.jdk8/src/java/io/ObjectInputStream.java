    ...
    extends InputStream implements ObjectInput, ObjectStreamConstants
{
     ...
     */
     ...
     private static boolean isClassCacheInitialized = false;
     static {
 ...
 public Boolean run() {
    ...
    private static final Map<?, Map<Class<?>, Class<?>>> serializationValidatorFactory = 
        (Map<?, Map<Class<?>, Class<?>>>) AccessController.doPrivileged(new PrivilegedAction<Object>() {
            public Object run() {
                return System.getProperties().get("com.ibm.serialization.validation.factory.instance");
            }
        });

    private final Map<Class<?>, Class<?>> serializationValidator = 
        (serializationValidatorFactory == null) ? null : serializationValidatorFactory.get(this);
    ...
    private Object readObjectImpl(Class caller)
               throws ClassNotFoundException, IOException
    {
        if(serializationValidator != null) {
            serializationValidator.clear();
        }
        try {
            if (enableOverride) {
    ...
    protected Class<?> resolveClass(ObjectStreamClass desc)
        throws IOException, ClassNotFoundException
    {
        if (!isClassCacheInitialized) {
            synchronized(ObjectInputStream.class) {
                if (!isClassCacheInitialized) {
                    isClassCacheInitialized = true;
        ...
        throws IOException
    {
        ...
        try {
            cl = (serializationValidator == null) ? resolveClass(readDesc) : serializationValidator.get(resolveClass(readDesc));
            if (cl == null) {
                resolveEx = new ClassNotFoundException("null class");
