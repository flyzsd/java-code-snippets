    ...
    extends OutputStream implements ObjectOutput, ObjectStreamConstants
{
    ...
    void writeTypeString(String str) throws IOException {
        ...
        if (str == null) {
           ...
           int strHash = HandleTable.hash(str);
           if ((handle = handles.lookup(str, strHash)) != -1) {
               writeHandle(handle);
           } else {
               writeString(str, strHash, false);
           }
        }
    }
        ...
        throws IOException
    {
        ...
        try {
            ...
            int h;
            obj = subs.lookup(obj, HandleTable.hash(obj));
            int objHash = HandleTable.hash(obj);
            if (obj  == null) {
                writeNull();
                return;
            } else if (!unshared && (h = handles.lookup(obj, objHash)) != -1) {
                writeHandle(h);
                return;
            } else if (obj instanceof Class) {
                writeClass((Class) obj, objHash, unshared);
                return;
            } else if (obj instanceof ObjectStreamClass) {
                writeClassDesc((ObjectStreamClass) obj, objHash, unshared);
                return;
            }

            // check for replacement object
            Object orig = obj;
            int origHash = objHash;
            ...
            if (obj != orig) {
                objHash = HandleTable.hash(obj);
                subs.assign(orig, origHash, obj);
                if (obj == null) {
                    writeNull();
                    return;
                } else {
                   if (!unshared && (h = handles.lookup(obj, objHash)) != -1) {
                       writeHandle(h);
                       return;
                   } else if (obj instanceof Class) {
                       writeClass((Class) obj, objHash, unshared);
            ...
            if (desc.isPackedObject(obj))
            {
                ...
                writeString((String) obj, objHash, unshared);
            } else {
               int descHash = HandleTable.hash(desc);
               if (cl.isArray()) {
                   writeArray(obj, objHash, desc, descHash, unshared);
               } else if (obj instanceof Enum) {
                   writeEnum((Enum<?>) obj, objHash, desc, descHash, unshared);
               } else if (obj instanceof Serializable) {
                   writeOrdinaryObject(obj, objHash, desc, descHash, unshared);
               } else {
                   if (extendedDebugInfo) {
     ...
     */
    private void writeClass(Class<?> cl, int clHash, boolean unshared) throws IOException {
        bout.writeByte(TC_CLASS);
        ObjectStreamClass desc = ObjectStreamClass.lookup(cl, true);
        int descHash = HandleTable.hash(desc);
        writeClassDesc(desc, descHash, false);
        if (unshared)
            handles.assign(null, HandleTable.hash(null));
        else
            handles.assign(cl, clHash);
    }

    /**
     * Writes representation of given class descriptor to stream.
     */
    private void writeClassDesc(ObjectStreamClass desc, int descHash, boolean unshared)
        throws IOException
    {
        int handle;
        if (desc == null) {
            writeNull();
        } else if (!unshared && (handle = handles.lookup(desc, descHash)) != -1) {
            writeHandle(handle);
        } else if (desc.isProxy()) {
            writeProxyDesc(desc, descHash, unshared);
        } else {
            writeNonProxyDesc(desc, descHash, unshared);
        }
    }
     ...
     */
    private void writeProxyDesc(ObjectStreamClass desc, int descHash, boolean unshared)
        throws IOException
    {
        ...
        writeClassDesc(superDesc, HandleTable.hash(superDesc), false);
    }
     ...
     */
    private void writeNonProxyDesc(ObjectStreamClass desc, int descHash, boolean unshared)
        throws IOException
    {
     ...
     */
    private void writeString(String str, int strHash, boolean unshared) throws IOException {
        if (unshared)
            handles.assign(null, HandleTable.hash(null));
        else
            handles.assign(str, strHash);
        ...
        throws IOException
    {
        ...
        if (unshared)
            ...
            handles.assign(array, arrayHash);
        ...
        throws IOException
    {
        ...
        if (sdesc.forClass() == Enum.class)
            ...
            writeClassDesc(sdesc, HandleTable.hash(sdesc), false);
        if (unshared)
            handles.assign(null, HandleTable.hash(null));
        else
            handles.assign(en, enHash);
        String enName = en.name();
        writeString(enName, HandleTable.hash(enName), false);
    }
    ...
    private static class HandleTable {
         ...
         */
        int assign(Object obj, int objHash) {
            if (size >= next.length) {
                growEntries();
            }
            if (size >= threshold) {
         ...
         * no mapping found.
         */
        int lookup(Object obj, int objHash) {
            if (size == 0) {
                return -1;
            }
            int index = objHash % spine.length;
         ...
         */
        private void insert(Object obj, int objHash, int handle) {
            int index = objHash % spine.length;
            objs[handle] = obj;
        ...
        private void growSpine() {
            ...
            for (int i = 0; i < size; i++) {
                // XXX: Can we avoid recalculating these hashes by storing them instead?
                insert(objs[i], hash(objs[i]), i);
            }
        }
    ...
    private static class ReplaceTable {
         ...
         */
        void assign(Object obj, int objHash, Object rep) {
            int index = htab.assign(obj, objHash);
         ...
         */
        Object lookup(Object obj, int objHash) {
            int index = htab.lookup(obj, objHash);
